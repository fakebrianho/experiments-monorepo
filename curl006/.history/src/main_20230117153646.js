...
import projectState from '..public/fboMorph.theatre-project-state.json'
...
function theatre() {
	...
	//create project
	const project = getProject('fboMorph', { state: projectState })
	...
	const myFBO = sheet.object('FBO', {
		...
		position: types.compound({
			xPos: types.number(fbo.particles.position.x, { range: [-2, 2] }),
			yPos: types.number(fbo.particles.position.y, { range: [-2, 2] }),
			zPos: types.number(fbo.particles.position.z, { range: [-2, 2] }),
		}),
		currData: types.number(0, { range: [0, 10] }),
		freq: types.number(fbo.simulationMaterial.uniforms.uCurlFreq.value, {
			range: [0, 30],
		}),
		amp: types.number(fbo.simulationMaterial.uniforms.uCurlAmp.value, {
			range: [0, 15],
		}),
	})
	myFBO.onValuesChange((values) => {
		...
		fbo.simulationMaterial.uniforms.currData.value = currData

		const { xPos, yPos, zPos } = values.position
		fbo.particles.position.set(xPos, yPos, zPos)

		const freq = values.freq
		fbo.simulationMaterial.uniforms.uCurlFreq.value = freq

		const amp = values.amp
		fbo.simulationMaterial.uniforms.uCurlAmp.value = amp
	})
}

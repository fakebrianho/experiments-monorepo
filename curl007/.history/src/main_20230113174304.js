...
import initFbo from './initFBOModel'
...
function animate() {
	...
	// scene.children[2]
	// group.scale.set(3, 1, 1)
	// group.scale.x = Math.abs(Math.sin(time) * 0.5) + 1
	// group.scale.y = Math.abs(Math.sin(time) * 0.5) + 1
	// group.scale.z = Math.abs(Math.sin(time) * 0.5) + 1
